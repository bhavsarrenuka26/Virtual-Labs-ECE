...
const tabs = ["Info", "Demo", "Quiz", "Try Yourself", "Reference", "Lab Manual"];

const AssignmentDetails = () => {
  const { id } = useParams();
  const [activeTab, setActiveTab] = useState("Info");
  const [labData, setLabData] = useState(null);
  const [subjectRefs, setSubjectRefs] = useState([]);   // subject-level references
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // Fetch the lab
        const labRes = await fetch(`${import.meta.env.VITE_API_URL}/api/assignments/${id}`);
        if (!labRes.ok) throw new Error("Failed to fetch lab");
        const lab = await labRes.json();
        setLabData(lab);

        // Fetch all subjects to find this lab's subject and its references
        const subRes = await fetch(`${import.meta.env.VITE_API_URL}/api/assignments/subjects`);
        if (subRes.ok) {
          const subjects = await subRes.json();
          const currentSubject = subjects.find(s => s._id === lab.subjectId);
          if (currentSubject?.references?.length > 0) {
            setSubjectRefs(currentSubject.references);
          }
        }
      } catch (error) {
  ...
  const calculateScore = async () => {
    ...
    labData.quiz.forEach((question, index) => {
      if (selectedAnswers[index] === question.correctAnswerIndex) calculatedScore += 1;
    });
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL}/api/auth/submit-quiz`, {
        ...
        body: JSON.stringify({ assignmentId: labData._id, subjectId: labData.subjectId, score: calculatedScore }),
      });
      const data = await response.json();
      if (!response.ok) alert("Failed to submit score: " + data.message);
  ...
  if (isLoading) {
    return <div className="text-center py-5 mt-5"><h4>Loading Lab Data...</h4></div>;
  }

  if (!labData) {
    return <div className="text-center py-5 mt-5"><h4>Assignment not found in database..</h4></div>;
  }
  ...
  const renderTabContent = () => {
    switch (activeTab) {
      ...
      case "Info":
        return (
          <div>
            <h3 className="mb-4" style={{ color: "#102b51" }}>{labData.title}</h3>
            ...
            <p style={{ whiteSpace: "pre-wrap", lineHeight: "1.8" }}>{labData.info.theory}</p>
          </div>
        );
      ...
      case "Demo":
        return (
          <div className="text-center py-5">
            <div className="mb-4">
              <svg xmlns="http://www.w3.org/2000/svg" width="60" height="60" fill="#102b51"
                className="bi bi-play-circle-fill mb-3" viewBox="0 0 16 16">
                <path d="M16 8A8 8 0 1 1 0 8a8 8 0 0 1 16 0zM6.79 5.093A.5.5 0 0 0 6 5.5v5a.5.5 0 0 0 .79.407l3.5-2.5a.5.5 0 0 0 0-.814l-3.5-2.5z" />
              </svg>
              <h3 className="fw-bold" style={{ color: "#102b51" }}>Demonstration Video</h3>
              <p className="lead text-secondary mt-2">Watch the step-by-step practical demonstration.</p>
            </div>
            {labData.demoVideoUrl ? (
              <a href={labData.demoVideoUrl} target="_blank" rel="noreferrer"
                className="btn btn-lg fw-bold px-5 py-3 shadow mt-3"
                style={{ backgroundColor: "#102b51", color: "white", borderRadius: "8px" }}>
                Watch Video
              </a>
            ) : (
              <button disabled className="btn btn-lg fw-bold px-5 py-3 shadow mt-3 btn-secondary"
        ...
        if (!token) {
          return (
            <div className="text-center py-5">
              ...
              <p className="lead mb-4">You must be logged in to take the lab quizzes and record your score.</p>
              <a href="/login" className="btn btn-lg fw-bold" style={{ backgroundColor: "#102b51", color: "white" }}>
                Log In to Continue
              </a>
            </div>
          );
        ...
        return (
          <div>
            ...
            {labData.quiz.map((q, index) => (
              <div key={index} className="w-100 mb-4 p-3 p-md-4 bg-white rounded shadow-sm border">
                <p className="fw-bold fs-5">{index + 1}. {q.questionText}</p>
                <div className="ms-3 mt-3">
                  {q.options.map((opt, i) => {
                    let textColor = "text-dark";
                    if (showResult) {
                      if (i === q.correctAnswerIndex) textColor = "text-success fw-bold";
                      else if (selectedAnswers[index] === i && i !== q.correctAnswerIndex)
                    ...
                    return (
                      <div className="form-check mb-3" key={i}>
                        <input className="form-check-input" type="radio" name={`question-${index}`}
                          id={`q${index}-opt${i}`} checked={selectedAnswers[index] === i}
                          onChange={() => handleOptionSelect(index, i)} disabled={showResult}
                          style={{ cursor: showResult ? "not-allowed" : "pointer" }} />
                        <label className={`form-check-label ${textColor}`} htmlFor={`q${index}-opt${i}`}
                          style={{ cursor: showResult ? "not-allowed" : "pointer", fontSize: "1.1rem" }}>
                          {opt}
                        </label>
                      </div>
                    );
            ...
            <div className="mt-5 text-center">
              {!showResult ? (
                <button className="btn btn-lg px-5 py-2 shadow"
                  style={{ backgroundColor: "#102b51", color: "white", border: "none" }}
                  onClick={calculateScore}
                  disabled={Object.keys(selectedAnswers).length < labData.quiz.length}>
                  Submit Answers
                </button>
              ) : (
                <div className="p-5 bg-light rounded-4 border shadow-sm">
                  <h2 className="mb-3">Quiz Complete!</h2>
                  <h1 className={score >= labData.quiz.length / 2 ? "text-success fw-bold" : "text-danger fw-bold"}>
      ...
      case "Try Yourself":
        ...
        return (
          <div className="text-center py-5 animation-fade-in">
            <h4 className="mb-3 fw-bold" style={{ color: "#102b51" }}>Virtual Environment</h4>
            {hasSimulatorLink ? (
              <>
                <p className="mb-4 text-secondary lead">
                  Ready to test your knowledge? Click below to launch the online simulator or compiler.
                </p>
                <a href={labData.tryYourself.link} target="_blank" rel="noreferrer"
              ...
              <div className="alert alert-warning d-inline-block text-start text-dark border-warning shadow-sm rounded-4 p-4 mt-3"
      ...
      case "Reference":
        // Combine subject-level references + any lab-specific reference
        const allRefs = [
          ...subjectRefs,
          ...(labData.references || [])
        ];
        ...
        return (
          <div className="animation-fade-in">
            <h4 className="mb-4 fw-bold" style={{ color: "#102b51" }}>Study Resources</h4>

            {allRefs.length > 0 ? (
              <div className="d-flex flex-column gap-3">
                {allRefs.map((ref, idx) => (
                  <div key={idx} className="card border-0 rounded-4 p-4 shadow-sm"
                    style={{ backgroundColor: "#e2e8f0" }}>
                    <div className="d-flex align-items-center justify-content-between flex-wrap gap-3">
                      <div>
                        <h5 className="fw-bold mb-1" style={{ color: "#0f172a" }}>{ref.title}</h5>
                      </div>
              ...
              <div className="card border-0 rounded-4 p-4 shadow-sm" style={{ backgroundColor: "#e2e8f0" }}>
                <div className="d-flex align-items-center justify-content-between flex-wrap gap-3">
                  <div>
                    <h5 className="fw-bold mb-1" style={{ color: "#0f172a" }}>Course Reference Library</h5>
      ...
      case "Lab Manual":
        return (
          <div className="text-center py-5">
            <div className="mb-4">
              <svg xmlns="http://www.w3.org/2000/svg" width="60" height="60" fill="#102b51"
                className="bi bi-file-earmark-pdf-fill mb-3" viewBox="0 0 16 16">
              ...
              <h3 className="fw-bold" style={{ color: "#102b51" }}>Official Lab Manual</h3>
              <p className="lead text-secondary mt-2">
                Download the complete lab manual for detailed step-by-step instructions,
                expected outputs, and submission guidelines.
              </p>
            </div>
            {labData.labManualLink ? (
              <a href={labData.labManualLink} target="_blank" rel="noreferrer"
                className="btn btn-lg fw-bold px-5 py-3 shadow mt-3"
                style={{ backgroundColor: "#102b51", color: "white", borderRadius: "8px" }}>
  ...
  return (
    <div style={{
      backgroundImage: `url(${bgImage})`, backgroundSize: "600px",
      backgroundRepeat: "repeat", backgroundPosition: "top left",
      width: "100%", minHeight: "100vh",
      backgroundColor: "rgba(250, 253, 250, 0.8)", backgroundBlendMode: "overlay",
    }}>
      ...
      <div className="container mt-5">
        <div className="card border-0 rounded-4" style={{
          minHeight: "400px", backgroundColor: "#f3f4f6",
