...
export default function AdminDashboard() {
  ...
  const [editForm, setEditForm] = useState({
    demoVideoUrl: "", labManualLink: "", referenceLink: "", aim: "", theory: "",
  });
  ...
  const [newSubjectData, setNewSubjectData] = useState({ name: "", year: "SE", semester: 3 });
  ...
  const [openRefSubjectId, setOpenRefSubjectId] = useState(null);
  const [subjectRefs, setSubjectRefs] = useState({});
  const [newRefRow, setNewRefRow] = useState({ title: "", url: "" });

  const navigate = useNavigate();
  const yearToSemesters = { SE: [3, 4], TE: [5, 6], BE: [7, 8] };
  ...
  const handleDeleteSubject = async (subjectId) => {
    ...
    } catch { alert("Network error deleting subject."); }
    finally { setConfirmDelete({ type: null, id: null, name: "" }); }
  };

  //  Reference manager 
  const handleOpenRefs = (subject) => {
    setOpenRefSubjectId(subject._id);
    setSubjectRefs(prev => ({
      ...prev,
      [subject._id]: subject.references ? subject.references.map(r => ({ ...r })) : []
    }));
    setNewRefRow({ title: "", url: "" });
  };

  const handleAddRefRow = (subjectId) => {
    if (!newRefRow.title.trim() || !newRefRow.url.trim()) {
      return alert("Please enter both a title and a URL.");
    }
    setSubjectRefs(prev => ({
      ...prev,
      [subjectId]: [...(prev[subjectId] || []), { title: newRefRow.title, url: newRefRow.url }]
    }));
    setNewRefRow({ title: "", url: "" });
  };

  const handleRemoveRefRow = (subjectId, idx) => {
    setSubjectRefs(prev => ({
      ...prev,
      [subjectId]: prev[subjectId].filter((_, i) => i !== idx)
    }));
  };

  const handleSaveRefs = async (subjectId) => {
    const token = localStorage.getItem("token");
    try {
      const res = await fetch(
        `${import.meta.env.VITE_API_URL}/api/assignments/subjects/${subjectId}/references`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
          body: JSON.stringify({ references: subjectRefs[subjectId] }),
        }
      );
      if (res.ok) {
        setSubjects(prev =>
          prev.map(s => s._id === subjectId ? { ...s, references: subjectRefs[subjectId] } : s)
        );
        setOpenRefSubjectId(null);
      } else {
        alert("Failed to save references.");
      }
    } catch { alert("Network error saving references."); }
  };
  ...
  const handleDeleteLab = async (labId) => {
    ...
    try {
      ...
      if (res.ok) setLabs(prev => prev.filter(l => l._id !== labId));
      else alert("Failed to delete lab.");
    } catch { alert("Network error deleting lab."); }
    finally { setConfirmDelete({ type: null, id: null, name: "" }); }
  };
  ...
  const handleAddLab = async (subjectId) => {
    ...
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL}/api/assignments/add-lab`, {
        ...
        body: JSON.stringify({ title: newLabData.title, subjectId, aim: newLabData.aim, theory: newLabData.theory, quiz: quizQuestions }),
      });
      if (res.ok) { setLabs([...labs, await res.json()]); handleCancelAdd(); }
      else alert("Failed to add lab on the server.");
    } catch { alert("Failed to add lab due to network error."); }
  };
  ...
  const handleEditClick = (lab) => {
    ...
    setEditForm({
      ...
      referenceLink: lab.references?.length > 0 ? lab.references[0].url : "",
      aim: lab.info?.aim || "",
  ...
  const handleAddEditQuestion = () => {
    if (!editTempQuestion.questionText || !editTempQuestion.opt1 || !editTempQuestion.opt2)
      return alert("Please provide a question and at least 2 options.");
    setEditQuizQuestions([...editQuizQuestions, {
  ...
  const handleDeleteEditQuestion = (idx) => setEditQuizQuestions(editQuizQuestions.filter((_, i) => i !== idx));
  ...
  const handleSaveEdit = async (id) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setEditingId(null); setEditQuizQuestions([]);
      }
  ...
  return (
    <div style={{
      backgroundImage: `url(${bgImage})`, backgroundSize: "900px",
      backgroundRepeat: "repeat", backgroundPosition: "top left",
      width: "100%", minHeight: "100vh",
      backgroundColor: "rgba(255, 255, 254, 0.9)", backgroundBlendMode: "overlay",
    }}>
      <style>{`
        .btn-navy { background-color: #1a2238 !important; border-color: #1a2238 !important; color: #ffffff !important; }
        .btn-navy:hover { background-color: #253150 !important; border-color: #253150 !important; }
        .btn-outline-navy { background-color: transparent !important; border-color: #1a2238 !important; color: #1a2238 !important; }
        .btn-outline-navy:hover { background-color: #1a2238 !important; color: #ffffff !important; }
        .btn-dashed-navy { background-color: transparent !important; border-color: #1a2238 !important; border-style: dashed !important; color: #1a2238 !important; }
        .btn-dashed-navy:hover { background-color: #1a2238 !important; color: #ffffff !important; }
        ...
        .ref-row { display: flex; align-items: center; gap: 10px; padding: 8px 0; border-bottom: 1px solid #f1f5f9; }
          ...
          <div className="confirm-box" onClick={e => e.stopPropagation()}>
            ...
            <div className="d-flex gap-2 justify-content-end">
              <button className="btn btn-light btn-sm px-4"
                onClick={() => setConfirmDelete({ type: null, id: null, name: "" })}>Cancel</button>
              <button className="btn btn-sm px-4 fw-bold"
        ...
        <div className="card border-0 shadow-sm rounded-4 p-4 mb-5" style={{ backgroundColor: "#e2e8f0" }}>
          ...
          <div className="row g-2 align-items-end">
            <div className="col-12 col-md-5">
              <input type="text" className="form-control rounded-3" placeholder="e.g., Computer Networks Lab"
                value={newSubjectData.name} onChange={e => setNewSubjectData({ ...newSubjectData, name: e.target.value })} />
            </div>
            <div className="col-6 col-md-3">
              <select className="form-select rounded-3" value={newSubjectData.year}
                onChange={e => {
                  const y = e.target.value;
                  setNewSubjectData({ ...newSubjectData, year: y, semester: yearToSemesters[y][0] });
                }}>
            ...
            <div className="col-6 col-md-2">
              <select className="form-select rounded-3" value={newSubjectData.semester}
            ...
            <div className="col-12 col-md-2">
              <button className="btn btn-navy fw-bold w-100" onClick={handleAddSubject}>Create</button>
            </div>
          </div>
        </div>
        ...
        {groupedLabs.map(subject => (
          <div key={subject._id} className="mb-5 bg-white p-3 p-md-4 rounded-4 shadow-sm border">
            ...
            <div className="d-flex align-items-center gap-2 gap-md-3 mb-4 border-bottom pb-3 flex-wrap">
              <div className="subject-header d-flex align-items-center gap-2 gap-md-3 flex-grow-1 flex-wrap"
                onClick={() => navigate(`/subject/${subject._id}/labs`)}>
                <div className="rounded-circle p-2 text-white d-flex align-items-center justify-content-center flex-shrink-0"
              ...
              <div className="d-flex gap-2 flex-shrink-0">
                <button className="btn btn-sm btn-outline-navy fw-bold"
                  onClick={() => openRefSubjectId === subject._id
                    ? setOpenRefSubjectId(null)
                    : handleOpenRefs(subject)}>
                  {openRefSubjectId === subject._id ? "Close References" : "Add References"}
                </button>
            ...
            {openRefSubjectId === subject._id && (
              <div className="p-3 rounded-3 mb-4 animation-fade-in"
                style={{ background: "#f8fafc", border: "1px solid #e2e8f0" }}>
                <p className="fw-bold mb-3 text-uppercase" style={{ fontSize: "12px", letterSpacing: "0.05em", color: "#64748b" }}>
                  Subject-Level References
                  <span className="ms-2 text-secondary fw-normal normal-case" style={{ textTransform: "none", letterSpacing: 0, fontSize: "12px" }}>

                  </span>
                </p>

                {/* Existing refs */}
                {(subjectRefs[subject._id] || []).length > 0 ? (
                  <div className="mb-3">
                    {(subjectRefs[subject._id] || []).map((ref, idx) => (
                      <div key={idx} className="ref-row">
                        <span className="fw-bold text-dark" style={{ fontSize: "13px", minWidth: 0, flex: 1 }}>
                          {ref.title}
                        </span>
                        <a href={ref.url} target="_blank" rel="noreferrer"
                          className="text-primary text-truncate" style={{ fontSize: "12px", maxWidth: "200px" }}>
                          {ref.url}
                        </a>
                        <button className="delete-q-btn flex-shrink-0"
                          onClick={() => handleRemoveRefRow(subject._id, idx)}>
                           Remove
                        </button>
                      </div>
                    ))}
                  ...
                  <p className="text-muted fst-italic small mb-3">No references added yet.</p>
                )}
                ...
                <div className="row g-2 mb-3">
                  <div className="col-12 col-md-4">
                    <input type="text" className="form-control form-control-sm"
                      placeholder='Title e.g. "Reference Book 1"'
                      value={newRefRow.title}
                      onChange={e => setNewRefRow({ ...newRefRow, title: e.target.value })} />
                  </div>
                  <div className="col-12 col-md-6">
                    <input type="text" className="form-control form-control-sm"
                      placeholder="URL e.g. https://drive.google.com/..."
                      value={newRefRow.url}
                      onChange={e => setNewRefRow({ ...newRefRow, url: e.target.value })} />
                  </div>
                  <div className="col-12 col-md-2">
                    <button className="btn btn-sm btn-outline-navy w-100 fw-bold"
                      onClick={() => handleAddRefRow(subject._id)}>
                      + Add
                    </button>
                  </div>
                </div>
                ...
                <div className="d-flex gap-2">
                  <button className="btn btn-navy btn-sm px-4 fw-bold"
                    onClick={() => handleSaveRefs(subject._id)}>
                    Save References
                  </button>
                  <button className="btn btn-light btn-sm"
                    onClick={() => setOpenRefSubjectId(null)}>
                    Cancel
                  </button>
                </div>
              </div>
            ...
            <div className="row g-3 mb-3">
              {subject.assignments.map(lab => (
                <div key={lab._id} className="col-12">
                  <div className={`card border shadow-sm rounded-3 bg-light${editingId === lab._id ? "" : " lab-card"}`}
                    ...
                    <div className="card-body py-3 px-3 px-md-4">
                      <div className="d-flex align-items-center justify-content-between gap-2 flex-wrap">
                        <span className="fw-bold text-dark" style={{ flex: 1, minWidth: 0 }}>{lab.title}</span>
                        {editingId !== lab._id && (
                          <div className="d-flex gap-2 flex-shrink-0 edit-zone">
                            <button className="btn btn-sm btn-outline-navy"
                              onClick={e => { e.stopPropagation(); handleEditClick(lab); }}>Edit</button>
                            <button className="btn-delete-sm"
                              onClick={e => { e.stopPropagation(); setConfirmDelete({ type: "lab", id: lab._id, name: lab.title }); }}>
                              Delete
                            </button>
                          </div>
                        )}
                      ...
                      {editingId === lab._id && (
                        <div className="mt-3 edit-zone">
                          ...
                          <div className="p-3 bg-white border rounded mb-3">
                            <p className="fw-bold mb-3 text-uppercase" style={{ fontSize: "12px", letterSpacing: "0.05em", color: "#64748b" }}>Lab Content</p>
                            <div className="row g-2">
                              ...
                              <div className="col-12 col-md-4">
                                <label className="small fw-bold">Lab-Specific Reference Link</label>
                              ...
                              <div className="col-12">
                                ...
                                <textarea className="form-control" rows="2" value={editForm.aim}
                                  onChange={e => setEditForm({ ...editForm, aim: e.target.value })} />
                              </div>
                              <div className="col-12">
                                <label className="small fw-bold">Experiment Theory</label>
                                <textarea className="form-control" rows="4" value={editForm.theory}
                          ...
                          <div className="p-3 bg-white border rounded mb-3">
                            <div className="d-flex justify-content-between align-items-center mb-3 flex-wrap gap-2">
                              <p className="fw-bold mb-0 text-uppercase" style={{ fontSize: "12px", letterSpacing: "0.05em", color: "#64748b" }}>
                                Quiz Questions <span className="ms-2 badge bg-light text-dark border">{editQuizQuestions.length}</span>
                              </p>
                              {!showAddEditQuestion && (
                                ...
                                {editQuizQuestions.map((q, idx) => (
                                  <div key={idx} className="quiz-question-item">
                                    <div className="d-flex justify-content-between align-items-start gap-2">
                                      <div style={{ flex: 1, minWidth: 0 }}>
                                        ...
                                        <div className="d-flex flex-wrap gap-2 mt-1">
                                          {q.options.map((opt, oIdx) => (
                                            <span key={oIdx} className="badge rounded-2" style={{
                                      ...
                                      <button className="delete-q-btn flex-shrink-0" onClick={() => handleDeleteEditQuestion(idx)}>Remove</button>
                                    </div>
                                  </div>
                                ))}
                              </div>
                          ...
                          <div className="d-flex gap-2 flex-wrap">
                            <button className="btn btn-navy btn-sm px-4 fw-bold" onClick={() => handleSaveEdit(lab._id)}>Save All Changes</button>
            ...
            {activeSubjectId === subject._id ? (
              <div className="mt-4 p-3 p-md-4 rounded-4 shadow-sm animation-fade-in"
                ...
                <div className="mb-3">
                  ...
                  <textarea className="form-control" rows="2" value={newLabData.aim}
                    onChange={e => setNewLabData({ ...newLabData, aim: e.target.value })} />
                </div>
                <div className="mb-3">
                  <label className="small fw-bold text-dark mb-1">Theory / Explanation</label>
                  <textarea className="form-control" rows="3" value={newLabData.theory}
                    onChange={e => setNewLabData({ ...newLabData, theory: e.target.value })} />
                </div>
                ...
                <div className="p-3 bg-white rounded-3 border">
                  ...
                  <div className="d-flex justify-content-between align-items-center mt-3 flex-wrap gap-2">
                    ...
                    <button className="btn btn-sm btn-outline-navy fw-bold px-3" onClick={handleAddQuestionToList}>+ Add Question</button>
                  </div>
                </div>
                <div className="d-flex gap-2 mt-4 pt-3 border-top flex-wrap">
                  <button className="btn btn-navy fw-bold px-4 shadow-sm" onClick={() => handleAddLab(subject._id)}>Save Experiment & Quiz</button>
                  <button className="btn btn-light" onClick={handleCancelAdd}>Cancel</button>
                </div>
              </div>
            ) : (
              <button className="btn btn-sm btn-dashed-navy w-100 mt-2 py-2 fw-bold"
